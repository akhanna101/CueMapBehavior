function [] = Plot_CM_Py_1_Sess_V2(days,rat,varargin)
...
for i = find(d_use & rat_use)
    ...
    [Response] = Response_MAT(MAT);
    
    %This gets logicals in the 144 x n format
    L.R = Time2Vert(strncmp('R',MAT.Block,1),MAT);
    L.V = Time2Vert(strncmp('V',MAT.Block,1),MAT);
    L.H = Time2Vert(strncmp('H',MAT.Block,1),MAT);
    L.Reward = Time2Vert(MAT.Rewards,MAT);
    L.Post5 = Time2Vert(log_extend(MAT.Rewards,5),MAT);
    L.Post_Rew = Time2Vert(log_extend(MAT.Rewards,post_rew),MAT);
    L.Post_Rew_Rem = Time2Vert(~log_extend(MAT.Rewards,post_rew),MAT);
...
if ~plot_tr || plot_rw
    ...
    for j = 1:3
        ...
        switch j
            ...
            case 2
                curr_plot = lmean(Response,L.R & L.Post_Rew);
                %keep clims the same
            case 3
                curr_plot = lmean(Response,L.R & ~L.Post_Rew);
                clims = [0 max(curr_plot)];
    ...
    figure
    b = bar3(reshape(lmean(Response,L.R & ~L.Post_Rew),12,12));
    for k = 1:length(b)
        zdata = b(k).ZData;
        b(k).CData = zdata;
        b(k).FaceColor = 'interp';
